...
import { RoadmapManipulationHandler } from './handlers/roadmap-manipulation.js';

// Initialize database asynchronously
let db: DatabaseAdapter;
let handlers: PDLFunctionHandlers;
let roadmapHandlers: RoadmapFunctionHandlers;
let manipulationHandlers: RoadmapManipulationHandler;
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      // ==================== ROADMAP MANIPULATION TOOLS ====================
      {
        name: 'manipulate_roadmap',
        description: 'Insert, delete, or reorganize roadmap phases, sprints, and tasks',
        inputSchema: {
          type: 'object',
          properties: {
            project_name: {
            ...
            operation: {
              type: 'string',
              enum: [
                'insert_phase', 'delete_phase', 'reorganize_phases',
                'insert_sprint', 'delete_sprint', 'reorganize_sprints',
                'insert_task', 'delete_task', 'reorganize_tasks',
                'bulk_update_tasks'
              ],
              description: 'Operation to perform',
            },
            // For phase operations
            phase_data: {
              type: 'object',
              description: 'Data for phase operations',
              properties: {
                ...
                reassign_sprints_to: { type: 'string' },
            ...
            sprint_data: {
              type: 'object',
              description: 'Data for sprint operations',
              properties: {
            ...
            task_data: {
              type: 'object',
              description: 'Data for task operations',
              properties: {
                task_id: { type: 'string' },
                sprint_id: { type: 'string' },
                description: { type: 'string' },
    ...
    switch (name) {
      ...
      case 'manipulate_roadmap': {
        ...
        switch (operation) {
          // Phase operations
          case 'insert_phase':
            result = await manipulationHandlers.insertRoadmapPhase(
              args.project_name as string,
              args.phase_data as any
            );
            break;
          case 'delete_phase':
            result = await manipulationHandlers.deleteRoadmapPhase(
              args.project_name as string,
              args.phase_data?.phase_id as string,
              args.phase_data?.reassign_sprints_to as string
            );
            break;
          case 'reorganize_phases':
            result = await manipulationHandlers.reorganizeRoadmapPhases(
              args.project_name as string,
              args.phase_data?.phase_order as string[]
            );
            break;

          // Sprint operations
          case 'insert_sprint':
            result = await manipulationHandlers.insertSprint(
              args.project_name as string,
              args.sprint_data?.phase_id as string,
              args.sprint_data as any
            );
            break;
          case 'delete_sprint':
            result = await manipulationHandlers.deleteSprint(
              args.project_name as string,
              args.sprint_data?.sprint_id as string,
              args.sprint_data?.reassign_tasks_to as string
            );
            break;
          case 'reorganize_sprints':
            result = await manipulationHandlers.reorganizeSprints(
              args.project_name as string,
              args.sprint_data?.movements as any[]
            );
            break;

          // Task operations
          case 'insert_task':
            result = await manipulationHandlers.insertTask(
              args.project_name as string,
              args.task_data?.sprint_id as string,
              args.task_data as any
            );
            break;
          case 'delete_task':
            result = await manipulationHandlers.deleteTask(
              args.project_name as string,
              args.task_data?.task_id as string
            );
            break;
          case 'reorganize_tasks':
            result = await manipulationHandlers.reorganizeTasks(
              args.project_name as string,
              args.task_data?.movements as any[]
            );
            break;
          case 'bulk_update_tasks':
            result = await manipulationHandlers.bulkUpdateTaskStatus(
              args.project_name as string,
              args.task_data?.bulk_updates as any[]
            );
            break;
...
async function main(): Promise<void> {
  try {
    ...
    manipulationHandlers = new RoadmapManipulationHandler(db.getUnderlyingDatabase() as any);
